...
class BillersTest {
    ...
    void testViewABiller() throws Exception {
        ...
        Biller mockBiller = new Biller();
        mockBiller.setBillerSystemId(billerSystemId);

        when(billerBackOfficeService.fetchBillerById(billerSystemId)).thenReturn(mapBillerEntityToDTO(mockBiller));

        mockMvc.perform(get(BILLERS_API_V1 + "get-biller-" + billerSystemId + "-details")
                        .header("Authorization", testHelper.retrieveValidAccessToken()))
                .andExpect(status().isOk());

        verify(billerBackOfficeService).fetchBillerById(billerSystemId);
    }

    public BillerDto mapBillerEntityToDTO(Biller biller) {
        BillerDto billerDto = new BillerDto();
        BeanUtils.copyProperties(biller, billerDto);
        return billerDto;
    }
}
